...
download_dnstt() {
    ...
    # Try to download pre-built
    if [[ "$ARCH" == "x86_64" ]]; then
        wget -q -O "$DNSTT_SERVER" "https://github.com/Official123-12/slowdns-ultra/releases/download/v1.0/dnstt-server-amd64" 2>/dev/null
        wget -q -O "$DNSTT_CLIENT" "https://github.com/Official123-12/slowdns-ultra/releases/download/v1.0/dnstt-client-amd64" 2>/dev/null
    elif [[ "$ARCH" == "aarch64" ]]; then
        wget -q -O "$DNSTT_SERVER" "https://github.com/Official123-12/slowdns-ultra/releases/download/v1.0/dnstt-server-arm64" 2>/dev/null
        wget -q -O "$DNSTT_CLIENT" "https://github.com/Official123-12/slowdns-ultra/releases/download/v1.0/dnstt-client-arm64" 2>/dev/null
    fi
    
    # If download failed, build from source
    if [[ ! -f "$DNSTT_SERVER" ]] || [[ ! -s "$DNSTT_SERVER" ]]; then
        print_warning "Pre-built download failed, building from source..."
